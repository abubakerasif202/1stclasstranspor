...
describe('site essentials', () => {
  ...
  it('never simulates form delivery when an endpoint is missing', async () => { const result = await submitQuoteForm({ firstName: 'Ava', lastName: 'Lee', email: 'ava@example.test', phone: '0431604240', pickupLocation: 'Sydney', deliveryLocation: 'Canberra', serviceType: 'Freight', freightType: 'Pallet', dangerousGoods: false, consent: true }); expect(result).toEqual({ success: false, message: expect.stringMatching(/not configured/i) }) })
})
